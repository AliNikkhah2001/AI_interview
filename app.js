...
function technologySearchText(technology) {
  return [technology.name, technology.category, technology.kind, technology.languages.join(" "), technology.deployment,
    ...
    technology.alternatives.join(" "), technology.flow.join(" "), technology.mental_model,
    ...(technology.architecture || []).flatMap(item => [item.label, item.detail]),
    ...(technology.tutorial || []).flatMap(item => [item.title, item.body])].join(" ").toLowerCase();
}
...
function technologyArchitecture(technology) {
  const architecture = technology.architecture || [];
  const dataPlane = architecture.slice(0, 3);
  const boundaries = architecture.slice(3);
  return `<div class="tech-anatomy" role="img" aria-label="${esc(technology.name)} framework structure"><div class="anatomy-lane"><strong>Data plane</strong><div class="anatomy-path">${dataPlane.map((item, index) => `${index ? '<span aria-hidden="true">→</span>' : ""}<div><small>${esc(item.label)}</small><b>${esc(item.detail)}</b></div>`).join("")}</div></div><div class="anatomy-boundaries">${boundaries.map(item => `<div><small>${esc(item.label)}</small><p>${esc(item.detail)}</p></div>`).join("")}</div></div>`;
}
function technologyTutorial(technology) {
  const lessons = technology.tutorial || [];
  const sources = technology.sources || [{label: `${technology.name} official documentation`, url: technology.source_url}];
  return `<section class="tech-teaching"><div><p class="eyebrow">MENTAL MODEL</p><p class="mental-model">${esc(technology.mental_model || technology.summary)}</p></div><div class="tech-lesson-grid">${lessons.map(lesson => `<article><h5>${esc(lesson.title)}</h5><p>${esc(lesson.body)}</p></article>`).join("")}</div><div class="tech-source-list"><strong>Primary sources</strong>${sources.map(source => `<a href="${esc(source.url)}" target="_blank" rel="noreferrer">${esc(source.label)} ↗</a>`).join("")}</div></section>`;
}
...
function renderTechnologyComparison() {
  ...
  $("#technologyComparison").innerHTML = `<p class="comparison-question"><strong>Decision question:</strong> ${esc(category.question)}</p><div class="table-scroll"><table><caption>${esc(category.title)} comparison</caption><thead><tr><th>Technology</th><th>Architecture and role</th><th>Languages</th><th>Deployment boundary</th><th>State / trust boundary</th><th>Choose when</th><th>Reject when</th></tr></thead><tbody>${technologies.map(technology => `<tr><th scope="row"><button class="table-tech-link" data-compare-tech="${esc(technology.id)}">${esc(technology.name)}</button></th><td><strong>${esc(technology.kind)}</strong><br>${esc(technology.mental_model)}</td><td>${esc(technology.languages.join(", "))}</td><td>${esc(technology.deployment)}</td><td>${esc(technology.architecture?.find(item => item.label === "State and trust boundary")?.detail || technology.failure_mode)}</td><td>${esc(technology.choose_when)}</td><td>${esc(technology.avoid_when)}</td></tr>`).join("")}</tbody></table></div>`;
...
function renderTechnologies() {
  ...
  $("#technologyGrid").innerHTML = technologies.length ? technologies.map(technology => {
    ...
    return `<details class="technology-card" id="technology-${esc(technology.id)}" data-technology-card="${esc(technology.id)}"><summary><span class="tech-logo"><img src="${esc(technology.logo_url)}" alt="${esc(technology.name)} logo" loading="lazy"><span hidden aria-hidden="true">${esc(technology.logo_fallback)}</span></span><span class="tech-title"><span class="cat">${esc(technology.category)}</span><strong>${esc(technology.name)}</strong><small>${esc(technology.kind)}</small></span><span class="card-chevron" aria-hidden="true">＋</span></summary><div class="technology-body"><p class="tech-summary">${esc(technology.summary)}</p><div class="tech-badges"><span>${esc(technology.languages.join(" · "))}</span><span>${esc(technology.deployment)}</span></div><h4>Abstract execution path</h4>${technologyFlow(technology)}<h4>Framework structure and design</h4>${technologyArchitecture(technology)}${technologyTutorial(technology)}<div class="selection-grid"><div><strong>Choose when</strong><p>${esc(technology.choose_when)}</p></div><div><strong>Reject when</strong><p>${esc(technology.avoid_when)}</p></div><div><strong>Primary failure</strong><p>${esc(technology.failure_mode)}</p></div><div><strong>Compare with</strong><p>${esc(technology.alternatives.join(" · "))}</p></div></div><details class="tech-quickstart"><summary>Language and minimal use</summary><pre><code>${esc(technology.quickstart)}</code></pre><p>Run this as an orientation, then use the cited official documentation for the exact installed version.</p></details><div class="tech-actions"><a href="${esc(technology.source_url)}" target="_blank" rel="noreferrer">Official documentation ↗</a><button data-tech-practice="${esc(technology.id)}">Practice medium → very hard</button>${design ? `<button data-tech-design="${esc(design.id)}">Open ${esc(design.title)}</button>` : ""}</div></div></details>`;
